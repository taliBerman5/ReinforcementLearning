...
void Planner::EvaluationLoop(DSPOMDP *model, World* world, Belief* belief,
		...
		int start_run, string solver_type) {
...
int Planner::RunEvaluation(int argc, char *argv[]) {
	 ...
	 * =========================*/
	//srand(time(NULL));  //added random numbers
	 ...
	 * Display parameters
	 * =========================*/
	cout << "Solver type "<< solver_type<< "\n";
	DisplayParameters(options, model);

	/* =========================
	 * run evaluation
	 * =========================*/
	string file_name = solver_type+"_"+typeid(*model).name()+"_runAlone_longTimeForFirst.txt"; //reward0_minus100
	myfile.open (file_name);
	myfile << "round__discounted_reward/round_undiscounted_reward\n";
	
	EvaluationLoop(model, world, belief, belief_type, solver, logger,
			options, main_clock_start, num_runs, start_run, solver_type);
