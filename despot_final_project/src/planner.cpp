...
bool Planner::RunStep(Solver* solver, World* world, Logger* logger, int different_time) {
	...
	ACT_TYPE action = solver->Search(different_time).action;
...
void Planner::PlanningLoop(Solver*& solver, World* world, Logger* logger, string solver_type) {
	for (int i = 0; i < Globals::config.sim_len; i++) {
		int different_time = 0;
		if ((i == 0) & (solver_type=="POMCP")){
			different_time = 1;
			cout <<"test"; //change to 1 when i want to check different time for the first step
		} 
		bool terminal = RunStep(solver, world, logger, different_time);
		if (terminal)
			break;
...
int Planner::RunPlanning(int argc, char *argv[]) {
	...
	round_=0; step_=0;
	PlanningLoop(solver, world, logger, solver_type);
	logger->EndRound();
