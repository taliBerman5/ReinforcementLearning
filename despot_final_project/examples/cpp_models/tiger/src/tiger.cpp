...
class TigerPOMCPPrior: public despot::POMCPPrior {
private:
    const despot::Tiger* tiger_model_;
public:
    TigerPOMCPPrior(const despot::DSPOMDP* model) :
            POMCPPrior(model),
            tiger_model_(static_cast<const despot::Tiger*>(model)) {
				
    }
    void ComputePreference(const despot::State& state) {
        ComputePreference((despot::TigerState&)state);
    }
    void ComputePreference(const despot::TigerState& state) {
        ...
        int count_diff = 0;
        for (int i = history_.Size() - 1;
             i >= 0 && history_.Action(i) == despot::Tiger::LISTEN; i--)
            count_diff += history_.Observation(i) == despot::Tiger::LEFT ? 1 : -1;

        if (count_diff >= 2)
            preferred_actions_.push_back(1);
        else if (count_diff <= -2)
            preferred_actions_.push_back(0);
        else
...
// despot::POMCPPrior* Tiger::CreatePOMCPPrior(string name) const {
