...
with tf.Session() as sess:
    ...
    saver.restore(sess, r"C:\Users\talib\Desktop\computer science\third year\model")
    ...
    for episode in range(max_episodes):
        ...
        for step in range(max_steps):
            ...
            if done:
                if episode > 10:
                    # Check if solved
                    average_rewards = np.mean(episode_rewards[(episode - 10):episode + 1])
                print("Episode {} Reward: {} Average over 10 episodes: {}".format(episode, episode_rewards[episode],
                                                                                   round(average_rewards, 2)))
                if average_rewards > -150 and average_rewards > last_average_rewards:
                    last_average_rewards = average_rewards

                    if last_average_rewards > -150:
                        solved = True
...
def simulate(env):
    ...
    counter = 1
    while not done:
        state_str = f"theta: {env.env.state[0]}, theta dot: {env.env.state[1]}"
        actions_distribution = sess.run(policy.actions_distribution, {policy.state: state})
        action = np.random.choice(np.arange(len(actions_distribution)), p=actions_distribution)
        state, r, done, _ = env.step(action)
        next_state_str = f"theta: {env.env.state[0]}, theta dot: {env.env.state[1]}"
        sum_of_reward += r
        print(f"({counter}). action: {action}, {state_str}, {next_state_str}, {r}")
        steps += 1
        counter += 1
